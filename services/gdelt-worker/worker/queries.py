...
def derived_queries(settings: Settings) -> list[tuple[str, str]]:
    ...
    return [
        (
            "coverage_episodes",
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'coverage_episodes')}
            PARTITION BY DATE(first_mention_at)
            CLUSTER BY cluster_id, outlet_id AS
        ...
        (
            "event_clusters",
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'event_clusters')} AS
            {article_context}
            , theme_entities AS (
              SELECT
                global_event_id,
        ...
        (
            ...
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'outlet_metrics_daily')}
            PARTITION BY metric_date
            CLUSTER BY outlet_id AS
            SELECT
        ...
        (
            ...
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'outlet_metrics_monthly')} AS
            SELECT
        ...
        (
            ...
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'entity_metrics_daily')}
            PARTITION BY metric_date
            CLUSTER BY entity_label AS
            {article_context}
            SELECT
        ...
        (
            ...
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'entity_metrics_monthly')} AS
            SELECT
              DATE_TRUNC(metric_date, MONTH) AS metric_month,
              entity_label,
        ...
        (
            ...
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'theme_metrics_monthly')} AS
            {article_context}
            SELECT
        ...
        (
            "propagation_edges",
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'propagation_edges')} AS
            WITH ranked AS (
              SELECT
                global_event_id,
                outlet_id,
        ...
        (
            "ghosting_flags",
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'ghosting_flags')} AS
            SELECT
        ...
        (
            "divergence_flags",
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'divergence_flags')} AS
            SELECT
        ...
        (
            ...
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'local_vs_national_monthly')} AS
            SELECT
        ...
        (
            "seasonality_rollups",
            f"""
            CREATE OR REPLACE TABLE {table(project, derived, 'seasonality_rollups')} AS
            SELECT
